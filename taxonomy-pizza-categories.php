...
*/
add_action('init', 'wp_pizzeria_categories_custom_taxonomy');

function wp_pizzeria_categories_custom_taxonomy() {
	//setup labels
	...
	register_taxonomy(
		...
		array(
			...
			'show_ui' => true,
			'query_var' => true,
			'rewrite' => array( 'slug' => 'pizza-category' )
		)
	);
}

add_action('wp_pizzeria_category_add_form_fields', 'wp_pizzeria_category_image_add');

function wp_pizzeria_category_image_add($taxonomy) { ?>
...
add_action('wp_pizzeria_category_edit_form_fields','wp_pizzeria_category_image_edit');

function wp_pizzeria_category_image_edit($taxonomy) { ?>
<tr class="form-field">
...
add_action('edit_term','wp_pizzeria_category_image_save');
add_action('create_term','wp_pizzeria_category_image_save');

function wp_pizzeria_category_image_save($term_id){
    if(isset($_POST['pizza_category-image'])){
    	$category_images = maybe_unserialize( get_option( 'wp_pizzeria_category_images' ) );
    	if ( !is_array( $category_images ) || !$category_images )
    		$category_images = array();
    	$category_images[$term_id] = $_POST['pizza_category-image'];
		update_option( 'wp_pizzeria_category_images', maybe_serialize( $category_images ) );
    }
}
