...
*/
add_action('init', 'wp_pizzeria_beverage_categories_custom_taxonomy');

function wp_pizzeria_beverage_categories_custom_taxonomy() {
	//setup labels
	...
	register_taxonomy(
		'wp_pizzeria_beverage_category',
		array('wp_pizzeria_beverage'), 
		array(
			'hierarchical' => true,
			'labels' => $labels_taxonomy,
			'show_ui' => true,
			'query_var' => true,
			'rewrite' => array( 'slug' => 'beverage-category' )
		)
	);
}

add_action('wp_pizzeria_beverage_category_add_form_fields', 'wp_pizzeria_beverage_category_image_add');

function wp_pizzeria_beverage_category_image_add($taxonomy) { ?>
<div class="form-field">
	<label for="beverage_category-image"><?php _e('Image', 'wp_pizzeria'); ?></label>
...
add_action('wp_pizzeria_beverage_category_edit_form_fields','wp_pizzeria_beverage_category_image_edit');

function wp_pizzeria_beverage_category_image_edit($taxonomy) { ?>
<tr class="form-field">
	...
	<td>
		...
		<input type="text" class="tag-image" name="beverage_category-image" id="beverage_category-image" value="<?php echo $category_images[$taxonomy->term_id]; ?>" />
		<br />
...
add_action('edit_term','wp_pizzeria_beverage_category_image_save');
add_action('create_term','wp_pizzeria_beverage_category_image_save');

function wp_pizzeria_beverage_category_image_save($term_id){
    if(isset($_POST['beverage_category-image'])){
    	$category_images = maybe_unserialize( get_option( 'wp_pizzeria_beverage_category_images' ) );
    	if ( !is_array( $category_images ) || !$category_images )
    		$category_images = array();
    	$category_images[$term_id] = $_POST['beverage_category-image'];
		update_option( 'wp_pizzeria_beverage_category_images', maybe_serialize( $category_images ) );
    }
}
